...
public sealed class MetricPollJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning(
                    "Metric poll job {JobKey}: definition {PollKey} not found for device {DeviceName}",
            ...
            _logger.LogDebug(
                "Metric poll completed for {DeviceName}/{PollKey}",
                deviceName, pollKey);
        }
        catch (OperationCanceledException) when (!context.CancellationToken.IsCancellationRequested)
        {
            ...
            _logger.LogWarning(
                "Metric poll job {JobKey} timed out waiting for SNMP response",
                jobKey);
        }
